...
const DEFAULT_SKILL_METADATA_PREVIEW_BYTES = 8 * 1024
...
function getSkillMetadataPreviewBytes(): number {
  const raw = Number(process.env.CLAUDE_CODE_SKILL_METADATA_READ_BYTES)
  return Number.isFinite(raw) && raw > 0
    ? Math.floor(raw)
    : DEFAULT_SKILL_METADATA_PREVIEW_BYTES
}

function hasTruncatedFrontmatter(
  preview: string,
  contentLength: number,
): boolean {
  if (preview.length >= contentLength) {
    return false
  }
...
export async function readSkillMetadataPreview(skillFilePath: string): Promise<{
  frontmatter: FrontmatterData
  markdownPreview: string
  contentLength: number
}> {
  const [{ size }, preview] = await Promise.all([
    stat(skillFilePath),
    readFilePreview(skillFilePath, getSkillMetadataPreviewBytes()),
  ])

  let parseInput = preview
  if (hasTruncatedFrontmatter(preview, size)) {
    parseInput = await getFsImplementation().readFile(skillFilePath, {
      encoding: 'utf-8',
    })
    logForDebugging(
      `[skills] Frontmatter preview truncated for ${skillFilePath}; fell back to full read`,
    )
  }

  let { frontmatter, content: markdownPreview } = parseFrontmatter(
    parseInput,
    skillFilePath,
  )
  ...
  if (
    parseInput === preview &&
    preview.length < size &&
    markdownPreview.trim().length === 0
  ) {
    parseInput = await getFsImplementation().readFile(skillFilePath, {
      encoding: 'utf-8',
    })
    ;({ frontmatter, content: markdownPreview } = parseFrontmatter(
      parseInput,
      skillFilePath,
    ))
    logForDebugging(
      `[skills] Metadata preview contained no body text for ${skillFilePath}; fell back to full read`,
    )
  }
...
export async function loadFullSkillMarkdownContent(
  skillFilePath: string,
): Promise<string> {
  const content = await getFsImplementation().readFile(skillFilePath, {
    encoding: 'utf-8',
  })
  return parseFrontmatter(content, skillFilePath).content
}
...
export function createSkillCommand({
  ...
  hasUserSpecifiedDescription: boolean
  markdownContent?: string
  markdownContentLoader?: () => Promise<string>
  contentLength: number
  allowedTools: string[]
  argumentHint: string | undefined
  argumentNames: string[]
  ...
  return {
    ...
    async getPromptForCommand(args, toolUseContext) {
      const resolvedMarkdownContent =
        markdownContent ?? (await markdownContentLoader?.()) ?? ''
  ...
  const results = await Promise.all(
    entries.map(async (entry): Promise<SkillWithPath | null> => {
      try {
        ...
        let metadata: Awaited<ReturnType<typeof readSkillMetadataPreview>>
        try {
          metadata = await readSkillMetadataPreview(skillFilePath)
        } catch (e: unknown) {
        ...
        const paths = parseSkillPaths(metadata.frontmatter)
        ...
        return {
          skill: createSkillCommand({
            ...
            skillName,
            markdownContentLoader: () =>
              loadFullSkillMarkdownContent(skillFilePath),
